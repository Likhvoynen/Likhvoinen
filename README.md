...
For r = 2 To lastRow
    ...
    ' Проверяем, что строка содержит "итог", но не "факторинг итог" и не "суды и прочие итог"
    If InStr(cellValue, "итог") > 0 And InStr(cellValue, "факторинг итог") = 0 And InStr(cellValue, "суды и прочие итог") = 0 Then
        ' Добавляем ячейку I из текущей строки в диапазон для суммирования
        If sumRange Is Nothing Then
            Set sumRange = Cells(r, "I")
        Else
            Set sumRange = Union(sumRange, Cells(r, "I"))
        End If
    End If
Next r

' Ищем строку с "Общий итог" и вставляем сумму
For r = 2 To lastRow
    If InStr(1, Cells(r, "A").Value, "Общий итог", vbTextCompare) > 0 Then
        If Not sumRange Is Nothing Then
            Cells(r, "I").Formula = "=SUM(" & sumRange.Address & ")"
        End If
        Exit For ' Останавливаем цикл после нахождения первой строки с "Общий итог"
    End If
Next r
...
For r = 2 To lastRow
    ...
    If InStr(cellValue, "итог") > 0 And InStr(cellValue, "факторинг итог") = 0 And InStr(cellValue, "суды и прочие итог") = 0 Then
        If sumRows = "" Then
            sumRows = "'Ю.В.'!I" & r
        Else
            sumRows = sumRows & ",'Ю.В.'!I" & r
        End If
    End If
Next r

' Вставляем формулу суммирования для строки с "Общий итог" с использованием INDIRECT
For r = 2 To lastRow
    If InStr(1, Cells(r, "A").Value, "Общий итог", vbTextCompare) > 0 Then
        If sumRows <> "" Then
            Cells(r, "I").Formula = "=SUM(INDIRECT(""" & sumRows & """))"
        End If
...
Привет! Напиши макрос, который будет вставлять данные из активного листа (исходный файл) на лист «план на месяц» в файл который будет открыт (целевой)
Определять с какой строки нужно вставлять данные будем по последнему заполненному значению в столбце АВ (целевой файл) 
Подтягивать значения нужно по названию столбцов: 
1) столбец U (исходный файл) вставляется в столбец С (целевой файл) и т.д
2) столбец v вставляется в столбец d
3) столбец w вставляется в столбец h
4) столбец y вставляется в столбец m
5) столбец AA вставляется в столбец O
6) столбец d вставляется в столбец r 
7) столбец I вставляется в столбец s
8) столбец g вставляется в столбец t
9) столбец h вставляется в столбец u
10) столбец n вставляется в столбец v
11) столбец q вставляется в столбец w
12) столбец t вставляется в столбец ae 
13) столбец x вставляется в столбец ai
14) столбец o вставляется в столбец ak
15) столбец j вставляется в столбец an 
16 столбец b вставляется в столбец ab 

Важно также сделать так, чтобы перед выполнением макроса выплывало диалоговое окно, куда можно было бы написать дату, по которой макрос будет определять какие данные перенести (дата будет находиться в столбце В (исходный файл). 
...
Sub TransferData()
    ...
    For Each cell In wsSource.Range("B2:B" & wsSource.Rows.Count)
        ...
        ' Отладочная информация (раскомментируйте для проверки)
        ' MsgBox "Значение: " & cell.Value & " | Тип: " & VarType(cell.Value) & " | Формат: " & cell.NumberFormat
    Next cell
...
Sub TransferData()
    ...
    With wsSource
        ' Принудительное преобразование столбца B в формат даты
        .Range("B2:B" & .Rows.Count).TextToColumns Destination:=.Range("B2"), DataType:=xlDelimited, _
            TextQualifier:=xlDoubleQuote, ConsecutiveDelimiter:=False, _
            Tab:=False, Semicolon:=False, Comma:=False, Space:=False, Other:=False, FieldInfo:=Array(1, 4)
    End With
    ...
    For Each cell In wsSource.Range("B2:B" & wsSource.Rows.Count)
        If Not IsEmpty(cell.Value) Then
            ...
            If IsDate(cell.Value) Then
                dateString = Format(cell.Value, "dd.mm.yyyy") ' Форматирование даты для сравнения
                Debug.Print "Проверяем ячейку: " & cell.Address & " Значение: " & dateString

                ' Сравнение с введенной датой
                If dateString = Format(sourceDate, "dd.mm.yyyy") Then
                    With wsTarget
                        .Cells(lastRowTarget, "C").Value = cell.Offset(0, 19).Value ' Столбец U -> C
                        .Cells(lastRowTarget, "D").Value = cell.Offset(0, 20).Value ' Столбец V -> D
                        .Cells(lastRowTarget, "H").Value = cell.Offset(0, 21).Value ' Столбец W -> H
                    ...
                    lastRowTarget = lastRowTarget + 1
                    rowsCopied = rowsCopied + 1
                    Debug.Print "Данные скопированы в строку: " & lastRowTarget
                End If
            Else
                Debug.Print "Ячейка не является датой: " & cell.Address
            End If
        End If
    Next cell
...
Sub TransferData()
    ...
    For Each cell In wsSource.Range("B2:B" & wsSource.Rows.Count)
        ' Проверка, является ли значение ячейки датой
        Debug.Print "Текущая ячейка: " & cell.Address & " Значение: " & cell.Value
        ...
        If Not IsDate(Trim(cell.Value)) Then
            Debug.Print "Ячейка не является датой: " & cell.Address & " Значение: " & cell.Value
        End If
    Next cell
